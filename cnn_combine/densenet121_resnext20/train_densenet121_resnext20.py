...
from modelzoo.densenet import DenseNet121
from modelzoo.ResNeXt_vd import ResNeXt20_vd_32x4d
# 网络结构定义

def MultiModel(image, visit):
    densenet121 = DenseNet121()
    resnext20_vd = ResNeXt20_vd_32x4d()
    output_image = densenet121.net(image, class_dim=1024)
    output_visit = resnext20_vd.net(visit, class_dim=1024)
...
def train():
    ...
    # 继续训练
    if os.path.isdir(os.path.join(config['model_path'], str(2))):
        fluid.io.load_params(exe, os.path.join(config['model_path'], str(2)), main_program=train_prog)
