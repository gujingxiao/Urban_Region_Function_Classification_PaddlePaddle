...
from cnn_combine.resnet50_dpn26.config_resnet50_dpn26 import config
from cnn_combine.utils.data_feeder import train_mapper, val_mapper, data_reader
from modelzoo.dpn import DPN26
from modelzoo.ResNet_vd import ResNet50_vd
# 网络结构定义

def MultiModel(image, visit):
    resnet50_vd = ResNet50_vd()
    dpn26 = DPN26()
    output_image = resnet50_vd.net(image, class_dim=1024)
    output_visit = dpn26.net(visit, class_dim=1024)
...
def train():
    ...
    # 继续训练
    # if os.path.isdir(os.path.join(config['model_path'], str(10))):
    #     fluid.io.load_params(exe, os.path.join(config['model_path'], str(10)), main_program=train_prog)

    def if_exist_ResNet50_vd(var):
       return os.path.exists(os.path.join('pretrained', var.name))

    fluid.io.load_vars(exe, 'pretrained', main_program=train_prog, predicate=if_exist_ResNet50_vd)
    ...
    for epoch in range(config['total_epochs']):
        ...
        for step, data in enumerate(train_reader()):
            ...
            if step % 100 == 0:
                now_time = time()
                print('Epoch %d - step %d/%d - loss %0.5f accuracy %0.5f time %0.4f' %
                      (epoch, step, iters, sum(train_losses) / len(train_losses), sum(train_accs) / len(train_accs), now_time - start))
        ...
        if test_acc > best_acc:
            fluid.io.save_params(exe, os.path.join(config['model_path'], str(epoch), '_', str(test_acc)), main_program=train_prog)
            best_acc = test_acc
