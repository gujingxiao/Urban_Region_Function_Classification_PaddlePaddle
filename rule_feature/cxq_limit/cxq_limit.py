...
def read_train():
    ...
    for i, file in enumerate(tr_files):
        ...
        for item in user_id:
            out.write(str(item) + ',' + str(area_label) + '\n')
    out.close()
...
def deal_train():
    ...
    time1 = time()
    df = pd.read_csv(root_dir + 'ruleFeature/{}/train_userid_label.csv'.format(base_folder), names=['user_id', 'label'])
    # 只保留只去过一个类别的用户
    print(len(df))
