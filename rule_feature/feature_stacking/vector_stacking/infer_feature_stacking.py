...
from rule_feature.feature_stacking.vector_stacking.train_feature_stacking import stackingCnnModel
from rule_feature.feature_stacking.vector_stacking.config_feature_stacking import config
from rule_feature.feature_stacking.vector_stacking.feature_stacking_data_feeder import infer_mapper, infer_data_reader
...
def infer():
    ...
    infer_reader = paddle.batch(
        reader=infer_data_reader(config['root_path']+'test.txt', infer_mapper, config['root_path']+"ruleFeature/stacking/test_npy_vector/"), batch_size=batch_size)

    f = open('vSubmit.txt', 'w+')
    count_num = np.zeros([1, 9])
    for i, data in enumerate(infer_reader()):
        ...
        for index in range(len(result[0])):
            ...
            count_num[0, category - 1] += 1
            f.write('%s\t%03d\n' % (data[index][2], category))
            np.save('/media/gujingxiao/f577505e-73a2-41d0-829c-eb4d01efa827/CityFunction/cnnStacking/stacking/vector/{}/{}.npy'.format(fold, data[index][2]),
                    item)
